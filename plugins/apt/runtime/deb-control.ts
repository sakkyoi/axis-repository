...
function readDebControlArchive(bytes: Uint8Array): { name: string; bytes: Uint8Array } {
  if (textDecoder.decode(bytes.slice(0, arGlobalHeader.length)) !== arGlobalHeader) {
    throw new ValidationError("APT artifact is not a Debian package archive");
  }
  ...
  while (offset + 60 <= bytes.byteLength) {
    ...
    if (trailer !== "`\n" || !Number.isFinite(size) || size < 0) {
      throw new ValidationError("APT artifact has an invalid Debian archive header");
    }

    const dataStart = offset + 60;
    const dataEnd = dataStart + size;
    if (dataEnd > bytes.byteLength) {
      throw new ValidationError("APT artifact Debian archive is truncated");
    }
...
function readControlFileFromTar(bytes: Uint8Array): string {
  ...
  while (offset + 512 <= bytes.byteLength) {
    ...
    if (!Number.isFinite(size) || size < 0) {
      throw new ValidationError("APT artifact control tar header is invalid");
    }

    const dataStart = offset + 512;
    const dataEnd = dataStart + size;
    if (dataEnd > bytes.byteLength) {
      throw new ValidationError("APT artifact control tar is truncated");
    }
...
export function parseDebianControl(text: string): DebControlMetadata {
  ...
  for (const rawLine of text.replace(/\r\n/g, "\n").split("\n")) {
    ...
    if (separator <= 0) {
      throw new ValidationError("APT artifact control metadata is invalid");
    }
