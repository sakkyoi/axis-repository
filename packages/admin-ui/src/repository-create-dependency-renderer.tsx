...
export function RepositoryDependencyFields({
  ...
  return (
    <div className="grid gap-4">
      {fields.map((field) => {
        if (field.kind === "signing-key") {
          return (
            <SigningKeyField
              key={field.name}
              field={field}
              repositoryName={repositoryName}
              signingKeyId={values[field.name] ?? ""}
              onSigningKeyChange={(signingKeyId) => update(field.name, signingKeyId)}
            />
          );
        }
        ...
        return (
          <ErrorState
            ...
            error={new Error(`${field.kind} is not supported by this admin UI.`)}
          />
        );
      })}
    </div>
