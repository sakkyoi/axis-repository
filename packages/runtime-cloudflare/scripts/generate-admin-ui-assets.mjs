...
async function main() {
  ...
  const source = [
    ...
    // Annotated rather than inferred: `as const` over a bundle this size asks
    // the compiler to write the whole of it out again as a type, which past a
    // certain length it refuses to do (TS7056). Nothing reads these as
    // literals, so nothing is lost by saying what they are.
    "export const adminUiAssetEntries: ReadonlyArray<readonly [string, { contentType: string; bodyBase64: string }]> = [",
